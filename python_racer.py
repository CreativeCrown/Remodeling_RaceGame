...
#Courseクラスのインスタンスを作成
cdata = Course(CMAX, CLEN, 3, 0, ["0'00.00"]*LAPS, DATA_LR, 
...
def main(): #メイン処理を行う関数
    ...
    while True: #無限ループ
        ...
        #描画用データをもとに道路を描く
        d_item.draw_road(board, cars, cdata, img_obj, img_sea, img_car)

        d_item.draw_text(str(int(cars[0].spd)) + "km/h", 680, 30, d_item.RED, fnt_m)   #速度を表示
        d_item.draw_text("lap {}/{}".format(cdata.laps, cdata.LAPS), 100, 30, d_item.WHITE, fnt_m)    #周回数を表示
        d_item.draw_text("time "+time_str(rec[0]), 100, 80, d_item.GREEN, fnt_s) #タイムを表示
        for i in range(cdata.LAPS):   #繰り返しで
            d_item.draw_text(cdata.laptime[i], 80, 130+40*i, d_item.YELLOW, fnt_s)  #ラップタイムを表示

        key = pygame.key.get_pressed()  #keyに全てのキーの状態を代入
        transi.game_transition(cars, cdata, d_item, img_title, img_car, se_crash, rec, recbk, key)
